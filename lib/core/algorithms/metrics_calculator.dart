...
class MetricsCalculator {
  ...
  static Future<Map<String, double>> calculateMetrics({
    ...
    try {
      ...
      if (forceData.isEmpty) {
        throw ArgumentError('Force data cannot be null or empty');
      }
      
      if (bodyWeight <= 0) {
        throw ArgumentError('Body weight must be positive, got: $bodyWeight');
      }
      
      if (forceData.data.isEmpty) {
        throw ArgumentError('Force data collection is empty');
      }
      
      // Validate data quality
      final invalidDataPoints = forceData.data.where((d) => 
        d.totalGRF.isNaN || d.totalGRF.isInfinite || d.totalGRF < 0
      ).length;
      
      if (invalidDataPoints > forceData.data.length * 0.1) {
        throw ArgumentError('Too many invalid data points: $invalidDataPoints/${forceData.data.length}');
      }
      ...
      try {
        ...
        AppLogger.error('Basic metrics calculation failed', e);
      ...
      try {
        ...
        AppLogger.error('Test-specific metrics calculation failed', e);
  ...
  static Map<String, double> _calculateBasicMetrics(
    ...
    if (data.isEmpty) {
      throw ArgumentError('Force data cannot be empty');
    }
    
    if (bodyWeightN <= 0) {
      throw ArgumentError('Body weight must be positive');
    }
    ...
    // Asimetri metrikleri - null safety
    final asymmetryValues = data.map((d) => d.asymmetryIndex).where((v) => !v.isNaN && v.isFinite).toList();
    final avgAsymmetry = asymmetryValues.isNotEmpty ? 
        asymmetryValues.reduce((a, b) => a + b) / asymmetryValues.length : 0.0;
    final maxAsymmetry = asymmetryValues.isNotEmpty ? 
        asymmetryValues.reduce(math.max) : 0.0;
    
    // Load distribution - division by zero check
    final leftForces = data.map((d) => d.leftGRF).where((v) => !v.isNaN && v.isFinite).toList();
    final rightForces = data.map((d) => d.rightGRF).where((v) => !v.isNaN && v.isFinite).toList();
    
    final avgLeftLoad = leftForces.isNotEmpty ? 
        leftForces.reduce((a, b) => a + b) / leftForces.length : 0.0;
    final avgRightLoad = rightForces.isNotEmpty ? 
        rightForces.reduce((a, b) => a + b) / rightForces.length : 0.0;
    
    final totalLoad = avgLeftLoad + avgRightLoad;
    final leftLoadPercentage = totalLoad > 0 ? (avgLeftLoad / totalLoad) * 100 : 50.0;
    final rightLoadPercentage = totalLoad > 0 ? (avgRightLoad / totalLoad) * 100 : 50.0;
    
    // Coefficient of variation (CV) - improved calculation
    final forceValues = data.map((d) => d.totalGRF).where((v) => !v.isNaN && v.isFinite).toList();
    final forceStdDev = _calculateStandardDeviation(forceValues);
    final forceCV = avgForce > 0 && !avgForce.isNaN ? forceStdDev / avgForce : 0.0;
    
    return {
      'peakForce': peakForce.isFinite ? peakForce : 0.0,
      'averageForce': avgForce.isFinite ? avgForce : 0.0,
      'minForce': minForce.isFinite ? minForce : 0.0,
      'relativeForce': peakForce > 0 && bodyWeightN > 0 ? peakForce / bodyWeightN : 0.0,
      'asymmetryIndex': avgAsymmetry,
      'maxAsymmetry': maxAsymmetry,
      'leftLoadPercentage': leftLoadPercentage,
      'rightLoadPercentage': rightLoadPercentage,
      'forceCoefficientOfVariation': forceCV.isFinite ? forceCV : 0.0,
  ...
  static double _calculateStandardDeviation(List<double> values) {
    ...
    final validValues = values.where((v) => !v.isNaN && v.isFinite).toList();
    if (validValues.isEmpty) return 0.0;
    
    final mean = validValues.reduce((a, b) => a + b) / validValues.length;
    if (mean.isNaN || !mean.isFinite) return 0.0;
    
    final variance = validValues.map((x) => (x - mean) * (x - mean)).reduce((a, b) => a + b) / validValues.length;
    if (variance.isNaN || !variance.isFinite || variance < 0) return 0.0;
