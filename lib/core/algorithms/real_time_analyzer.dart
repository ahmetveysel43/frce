...
class RealTimeAnalyzer {
  // Buffer parametreleri - optimized for memory efficiency
  static const int _maxBufferSize = 5000; // 5 saniye @ 1000Hz (memory optimized)
  ...
  static const int _batchProcessSize = 100; // Process data in batches
  ...
  Future<void> startAnalysis({
    ...
    try {
      ...
      _testType = testType;
      // Phase parameters will be calculated dynamically as needed
      // Custom phase params ignored for now - using adaptive thresholds
  ...
  void addForceData(ForceData data) {
    ...
    try {
      // Input validation
      if (data.totalGRF.isNaN || data.totalGRF.isInfinite || data.totalGRF < 0) {
        AppLogger.warning('Invalid force data dropped: ${data.totalGRF}');
        return;
      }
      ...
      // Buffer boyutunu kontrol et (sliding window)
      if (_dataBuffer.length > _maxBufferSize) {
        // Remove multiple old samples for efficiency
        final removeCount = _dataBuffer.length - _maxBufferSize + _batchProcessSize;
  ...
  void addForceDataBatch(List<ForceData> dataList) {
    ...
    try {
      ...
      final validData = dataList.where((data) => 
        !data.totalGRF.isNaN && data.totalGRF.isFinite && data.totalGRF >= 0
      ).toList();
      ...
      // Add all valid data
      _dataBuffer.addAll(validData);
      
      // Maintain buffer size efficiently
      if (_dataBuffer.length > _maxBufferSize) {
        final removeCount = _dataBuffer.length - _maxBufferSize;
        _dataBuffer.removeRange(0, removeCount);
      }
      
      // Update moving average with last data point
      if (validData.isNotEmpty) {
        _updateMovingAverage(validData.last.totalGRF);
        _performQualityChecks(validData.last);
      }
  ...
  void _startAnalysisTimer() {
    _analysisTimer = Timer.periodic(
      const Duration(milliseconds: _feedbackUpdateInterval),
  ...
  /// Moving average güncelle (thread-safe)
  void _updateMovingAverage(double force) {
    if (force.isNaN || force.isInfinite) return;
    ...
    // Current smoothed force with validation
    if (_movingAverageBuffer.isNotEmpty) {
      final sum = _movingAverageBuffer.reduce((a, b) => a + b);
      _state.currentSmoothedForce = sum.isFinite ? sum / _movingAverageBuffer.length : 0.0;
    } else {
      _state.currentSmoothedForce = 0.0;
    }
  }
  ...
  void _performRealTimeAnalysis() {
    ...
    try {
      ...
      if (_lastProcessTime != null) {
        final processingDelay = now.difference(_lastProcessTime!).inMilliseconds;
        if (processingDelay > 150) { // More than 150ms delay
          AppLogger.warning('Analysis processing delay: ${processingDelay}ms');
        }
      }
      _lastProcessTime = now;
      
      // Create safe copy of data for analysis
      final recentData = _dataBuffer.length > _phaseDetectionWindow
          ...
          : List<ForceData>.from(_dataBuffer);
      ...
      JumpPhase newPhase;
      try {
        newPhase = PhaseDetector.detectRealTimePhase(
        ...
        AppLogger.error('Phase detection failed', e);
        newPhase = _state.currentPhase; // Keep current phase
      }
  ...
  /// Analyzer'ı tamamen dispose et
  void dispose() {
    try {
      AppLogger.debug('🧹 RealTimeAnalyzer disposing...');
      ...
      AppLogger.debug('✅ RealTimeAnalyzer disposed');
    } catch (e) {
      AppLogger.error('RealTimeAnalyzer dispose hatası', e);
    }
  }
}
