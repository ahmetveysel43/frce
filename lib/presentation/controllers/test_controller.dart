...
class TestController extends GetxController {
  ...
  final _forceDataStreamController = StreamController<ForceData>.broadcast();
  final _metricsStreamController = StreamController<RealTimeMetrics>.broadcast();
  ...
  // Streams
  Stream<ForceData> get forceDataStream => _forceDataStreamController.stream;
  Stream<RealTimeMetrics> get metricsStream => _metricsStreamController.stream;
  ...
  void startTest() {
    ...
    _realTimeAnalyzer = RealTimeAnalyzer();
    _realTimeAnalyzer!.startAnalysis(
      bodyWeight: _measuredWeight.value!,
      testType: _selectedTestType.value!,
    );
    
    // Connect analyzer streams to controller streams
    _realTimeAnalyzer!.metricsStream.listen((metrics) {
  ...
  void _generateMockForceData() {
    ...
    // Emit to stream for real-time graph
    _forceDataStreamController.add(mockData);
    
    // Add to real-time analyzer
    _realTimeAnalyzer?.addForceData(mockData);
  ...
  ForceData _generateBalanceData(int timestamp, double t, double bodyWeight) {
    ...
    return ForceData.create(
      ...
      leftCopX: math.sin(t * 2) * 30 + (math.Random().nextDouble() - 0.5) * 20,
      leftCopY: math.cos(t * 1.5) * 40 + (math.Random().nextDouble() - 0.5) * 25,
      rightCopX: math.sin(t * 2.2) * 25 + (math.Random().nextDouble() - 0.5) * 15,
      rightCopY: math.cos(t * 1.8) * 35 + (math.Random().nextDouble() - 0.5) * 20,
    );
  }
  ...
  ForceData _generateGenericData(int timestamp, double t, double bodyWeight) {
    ...
    return ForceData.create(
      ...
      leftCopX: (math.Random().nextDouble() - 0.5) * 80,
      leftCopY: (math.Random().nextDouble() - 0.5) * 120,
      rightCopX: (math.Random().nextDouble() - 0.5) * 80,
      rightCopY: (math.Random().nextDouble() - 0.5) * 120,
    );
  }
  ...
  void _stopTest() {
    ...
    _realTimeAnalyzer?.stopAnalysis();
  ...
  @override
  void onClose() {
    AppLogger.info('🎮 TestController disposing...');
    
    // Cancel all timers
    _cancelAllTimers();
    
    // Close streams
    _forceDataStreamController.close();
