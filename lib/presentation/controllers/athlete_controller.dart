...
class AthleteController extends GetxController {
  ...
  // Statistics
  final _athleteStats = AthleteStatistics().obs;
  ...
  Future<void> loadAthleteTestHistory(String athleteId) async {
    ...
    try {
      ...
      for (final session in testSessions) {
        final metrics = await db.getTestResults(session['id'] as String);
        
        final result = TestResult(
          id: session['id'] as String,
          sessionId: session['id'] as String,
          athleteId: athleteId,
          testType: TestType.values.firstWhere((t) => t.name == session['testType']),
          testDate: DateTime.parse(session['testDate'] as String),
          duration: Duration(milliseconds: session['duration'] as int),
          status: TestStatus.values.firstWhere((s) => s.name == session['status']),
          metrics: metrics,
          notes: session['notes'] as String?,
          createdAt: DateTime.parse(session['createdAt'] as String),
        );
