...
class PerformTestUseCase {
  ...
  Future<TestExecutionResult> execute(TestExecutionParams params) async {
    ...
    } catch (e) { // ✅ FIXED: Removed unused stackTrace
  ...
  ForceData _generateBalanceForceData(int timestamp, double t, double bodyWeight, Athlete athlete) {
    ...
    return ForceData.create(
      timestamp: timestamp,
      leftGRF: (baseForce * (0.5 + asymmetry / 2)).toDouble(), // ✅ FIXED: .toDouble()
      rightGRF: (baseForce * (0.5 - asymmetry / 2)).toDouble(), // ✅ FIXED: .toDouble()
      leftCopX: swayX + (math.Random().nextDouble() - 0.5) * 8,
      leftCopY: swayY + (math.Random().nextDouble() - 0.5) * 10,
      rightCopX: swayX + (math.Random().nextDouble() - 0.5) * 8,
      rightCopY: swayY + (math.Random().nextDouble() - 0.5) * 10,
    );
  }
  ...
  ForceData _generateGenericForceData(int timestamp, double t, double bodyWeight) {
    ...
    return ForceData.create(
      timestamp: timestamp,
      leftGRF: (totalForce * (0.5 + asymmetry / 2)).toDouble(), // ✅ FIXED: .toDouble()
      rightGRF: (totalForce * (0.5 - asymmetry / 2)).toDouble(), // ✅ FIXED: .toDouble()
      leftCopX: (math.Random().nextDouble() - 0.5) * 60,
      leftCopY: (math.Random().nextDouble() - 0.5) * 80,
      rightCopX: (math.Random().nextDouble() - 0.5) * 60,
      rightCopY: (math.Random().nextDouble() - 0.5) * 80,
    );
  }
  ...
  ForceData _createForceDataWithAsymmetry(int timestamp, double totalForce, Athlete athlete) {
    ...
    return ForceData.create(
      timestamp: timestamp,
      leftGRF: leftGRF.toDouble(), // ✅ FIXED: .toDouble()
      rightGRF: rightGRF.toDouble(), // ✅ FIXED: .toDouble()
      leftCopX: (math.Random().nextDouble() - 0.5) * 80,
      leftCopY: (math.Random().nextDouble() - 0.5) * 100,
      rightCopX: (math.Random().nextDouble() - 0.5) * 80,
      rightCopY: (math.Random().nextDouble() - 0.5) * 100,
    );
  }
